...
<section class="py-8">
    <h2 class="text-3xl font-bold mb-4">{isAgenda ? 'Meeting Agendas' : 'Meeting Minutes'}{isCEO ? 'CEO Reports' : 'Meeting Minutes'}</h2>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        {#each meeting.slice(0, 3) as minute}
            <MeetingMinuteItem date={minute.date} fileLink={minute.fileLink} isAgenda={isAgenda} />
        {/each}
    </div>
    {#if !isAgenda}
